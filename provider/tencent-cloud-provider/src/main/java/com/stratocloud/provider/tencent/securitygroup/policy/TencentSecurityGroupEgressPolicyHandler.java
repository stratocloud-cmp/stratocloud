...
@Component
public class TencentSecurityGroupEgressPolicyHandler
        extends AbstractResourceHandler implements TransientResourceHandler {

    private final TencentCloudProvider provider;


    public TencentSecurityGroupEgressPolicyHandler(TencentCloudProvider provider) {
    ...
    public String getResourceTypeId() {
        return "TENCENT_CLOUD_SECURITY_GROUP_EGRESS_POLICY";
    }
    ...
    public Optional<SecurityGroupPolicy> describeEgressPolicy(ExternalAccount account, String externalId) {
    ...
    private ExternalResource toExternalResource(ExternalAccount account,
        ...
        String externalId = TencentSecurityGroupPolicyId.fromPolicy(
                securityGroupId,
                SecurityGroupPolicyDirection.egress,
                securityGroupPolicy
        ).toString();
        ...
        return new ExternalResource(
                ...
                getResourceTypeId(),
                externalId,
                "%s-出站规则-%s".formatted(securityGroup.getSecurityGroupName(), securityGroupPolicy.getPolicyIndex()),
    ...
    public List<ExternalResource> describeExternalResources(ExternalAccount account, Map<String, Object> queryArgs) {
        ...
        return client.describeSecurityGroupPolicies(SecurityGroupPolicyDirection.egress).stream().map(
                securityGroupPolicy -> toExternalResource(account, securityGroupPolicy)
        ).toList();
    }
    ...
    public void synchronize(Resource resource) {
        ...
        SecurityGroupPolicy policy = describeEgressPolicy(account, resource.getExternalId()).orElseThrow(
        ...
        RuntimeProperty cidrRuntimeProperty = RuntimeProperty.ofDisplayInList(
                "cidrBlock", "目标地址(IPv4)", policy.getCidrBlock(), policy.getCidrBlock()
        );

        RuntimeProperty ipv6CidrRuntimeProperty = RuntimeProperty.ofDisplayInList(
                "ipv6CidrBlock", "目标地址(IPv6)", policy.getIpv6CidrBlock(), policy.getIpv6CidrBlock()
        );
