...
@Component
public class TencentNicBackendToL7RuleHandler implements EssentialRequirementHandler {

    public static final String TYPE_ID = "TENCENT_NIC_BACKEND_TO_L7_RULE_RELATIONSHIP";
    private final TencentNicBackendHandler backendHandler;

    private final TencentL7ListenerRuleHandler ruleHandler;

    public TencentNicBackendToL7RuleHandler(TencentNicBackendHandler backendHandler,
    ...
    public List<ExternalRequirement> describeExternalRequirements(ExternalAccount account,
        ...
        for (RuleTargets rule : rules) {
            ...
            Optional<Backend> optionalBackend = Arrays.stream(rule.getTargets()).filter(
                    t -> Objects.equals(
                            t.getPrivateIpAddresses()[0],
                            backend.get().backend().getPrivateIpAddresses()[0]
                    )
            ).findAny();
