package com.stratocloud.provider.huawei.rocket;
...
@Component
public class HuaweiRocketMqHandler extends AbstractResourceHandler {

    private final HuaweiCloudProvider provider;

    public HuaweiRocketMqHandler(HuaweiCloudProvider provider) {
    ...
    public String getResourceTypeId() {
        return "HUAWEI_ROCKETMQ";
    }

    @Override
    public String getResourceTypeName() {
        return "华为云RocketMQ实例";
    }
    ...
    public Optional<ExternalResource> describeExternalResource(ExternalAccount account, String externalId) {
        Optional<InstanceDetail> instance = describeInstance(account, externalId);
        return instance.map(
                i->toExternalResource(account, i)
        );
    }

    public ExternalResource toExternalResource(ExternalAccount account, InstanceDetail instance) {
    ...
    public Optional<InstanceDetail> describeInstance(ExternalAccount account, String externalId) {
        if(Utils.isBlank(externalId))
            return Optional.empty();

        return provider.buildClient(account).rocket().describeInstance(externalId);
    }

    @Override
    public List<ExternalResource> describeExternalResources(ExternalAccount account, Map<String, Object> queryArgs) {
        ListInstancesRequest request = new ListInstancesRequest();
        return provider.buildClient(account).rocket().describeInstances(request).stream().map(
                i -> toExternalResource(account,i)
        ).toList();
    }
    ...
    public void synchronize(Resource resource) {
        ...
        var instance = describeInstance(account, resource.getExternalId()).orElseThrow(
                () -> new ExternalResourceNotFoundException("RocketMQ instance not found")
        );
