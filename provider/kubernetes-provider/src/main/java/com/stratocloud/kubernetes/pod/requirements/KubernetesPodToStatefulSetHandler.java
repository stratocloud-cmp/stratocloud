...
@Component
public class KubernetesPodToStatefulSetHandler implements ExclusiveRequirementHandler {

    private final KubernetesPodHandler podHandler;

    private final KubernetesStatefulSetHandler statefulSetHandler;

    public KubernetesPodToStatefulSetHandler(KubernetesPodHandler podHandler,
                                             KubernetesStatefulSetHandler statefulSetHandler) {
    ...
    public String getRelationshipTypeId() {
        return "KUBERNETES_POD_TO_STATEFUL_SET_RELATIONSHIP";
    }

    @Override
    public String getRelationshipTypeName() {
        return "K8s StatefulSet与Pod";
    }
    ...
    public List<ExternalRequirement> describeExternalRequirements(ExternalAccount account, ExternalResource source) {
        ...
        Optional<V1OwnerReference> ownerReference = KubeUtil.getOwnerReference(
                pod.get().getMetadata(), "StatefulSet"
        );
        ...
        return statefulSet.map(er -> List.of(
